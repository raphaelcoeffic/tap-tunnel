...
fn main() {
    eprintln!("tap-tunnel-proxy requires Linux.");
    std::process::exit(1);
}
...
mod proxy {
    ...
    use crate::ipc::{accept_stream, create_stream_listener};
    use crate::namespace::join_namespace;
    use crate::tap::{configure_interface, create_tap};
    use tap_tunnel::TapConfig;
    use tap_tunnel::protocol::{
        ClientHello, Message, ProxyCommand, ProxyConfig, ProxyResponse, decode_control,
        decode_message, encode_control, encode_frame, read_framed, read_framed_async, write_framed,
        write_framed_async,
    };
    ...
    fn perform_handshake(fd: &OwnedFd, config: &TapConfig, tap_mac: [u8; 6]) -> io::Result<()> {
        ...
        // Wrap raw FD in a UnixStream for framed I/O (dup so we don't consume the FD)
        ...
        // Receive ClientHello (length-framed)
        ...
        // Send ProxyConfig with proxy identity (length-framed)
        ...
        write_framed(&mut stream, &config_msg)?;
        debug!("sent ProxyConfig: {}", proxy_config);

        // Don't close the dup'd fd — let it drop naturally (it's a dup, original stays open)
        Ok(())
    }
    ...
    async fn run_proxy(
        ...
        // Convert frame socket FD to tokio UnixStream for framed I/O
        let frame_std =
            unsafe { std::os::unix::net::UnixStream::from_raw_fd(frame_fd.into_raw_fd()) };
        frame_std.set_nonblocking(true)?;
        let frame_stream = tokio::net::UnixStream::from_std(frame_std)?;
        let (frame_reader, mut frame_writer) = frame_stream.into_split();
        ...
        if let Some((IpAddr::V4(ipv4), _)) = config.peer_addr {
            ...
            write_framed_async(&mut frame_writer, &msg).await?;
        ...
        let (rt_conn, rt_handle, _) = rtnetlink::new_connection().map_err(|e| {
            io::Error::other(format!("failed to create rtnetlink connection: {}", e))
        })?;
        tokio::spawn(rt_conn);

        // Add initial routes from CLI args
        for (dest, prefix_len) in &config.peer_routes {
            match crate::route::add_route(&rt_handle, &config.interface_name, *dest, *prefix_len)
                .await
            {
                Ok(()) => debug!(
                ...
                Err(e) => {
                    error!("failed to add initial route {}/{}: {}", dest, prefix_len, e)
                }
            }
        }
    ...
    async fn handle_proxy_command(
        ...
        match cmd {
            ProxyCommand::AddRoute {
                ...
                match crate::route::add_route(rt_handle, iface_name, destination, prefix_len).await
                {
                    Ok(()) => {
                        debug!(
            ...
            ProxyCommand::RemoveRoute {
                ...
                match crate::route::remove_route(rt_handle, iface_name, destination, prefix_len)
                    .await
                {
                    Ok(()) => {
                        debug!(
    ...
    async fn run_frame_relay(
        mut tap: AsyncFdIo,
        mut frame_reader: tokio::net::unix::OwnedReadHalf,
        mut frame_writer: tokio::net::unix::OwnedWriteHalf,
        rt_handle: &rtnetlink::Handle,
        iface_name: &str,
        ...
        loop {
            tokio::select! {
                // TAP → IPC: Forward Ethernet frame with type prefix (length-framed)
                result = tap.read(&mut tap_buf) => {
                    ...
                    // Encode frame with type prefix, then length-frame it
                    let msg = encode_frame(&tap_buf[..n]);
                    trace!("[PROXY] TAP → IPC: {} bytes (frame: {})", msg.len(), n);
                    write_framed_async(&mut frame_writer, &msg).await?;
                }

                // IPC → TAP: Read length-framed message, decode and forward frame
                result = read_framed_async(&mut frame_reader) => {
                        ...
                        Ok(Message::Control(payload)) => {
                            ...
                            match decode_control::<ProxyCommand>(&payload) {
                                Ok(cmd) => {
                                    ...
                                    write_framed_async(&mut frame_writer, &response_msg).await?;
                                }
                                Err(e) => {
